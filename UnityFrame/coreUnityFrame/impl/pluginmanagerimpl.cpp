...
void PluginManagerImpl::init()
{
    ...
    QString envDirs = UNITYFRAME->getEnv("SQLITESTUDIO_PLUGINS");
    if (!envDirs.isNull())
...
{
    if (metaObject.value("gui").toBool(false) && !UNITYFRAME->isGuiAvailable())
    {
